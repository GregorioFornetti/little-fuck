...
describe('Testes de envio de mensagem / eventos pelo servidor', () => {
  ...
  describe('General events', () => {
    ...
    test('debug', (done) => {
      ...
      lobbyClientsSockets[0].on('debug', (lobby: Lobby) => {
        expect(lobby.lobbyId).toBe('1');
        expect(lobby.players).toHaveLength(1);
        expect(lobby.players[0].id).toBe('1');
        expect(lobby.players[0].name).toBe('player1');
      ...
      lobbyClientsSockets[1].on('debug', (lobby: Lobby) => {
        expect(lobby.lobbyId).toBe('1');
        expect(lobby.players).toHaveLength(1);
        expect(lobby.players[0].id).toBe('1');
        expect(lobby.players[0].name).toBe('player1');
      ...
      lobbyEmitter.General.emitDebug({
        lobbyId: '1',
        players: [
          {
            id: '1',
    ...
    test('debug event should throw an error if the server is not in debug mode', () => {
      process.env.DEBUG = '';
      const lobbyEmitter = joinLobby(lobbyClientsSockets, lobbyServerSockets);
      ...
      }).toThrow(new Error(i18n.t('COMMON.ERROR.NO_DEBUG_MODE')));
    });
  });
});
