...
describe('handleJoinLobbySuccess', () => {
  ...
  test('Deve guardar as informações do lobby caso o jogador não estivesse em nenhum antes', () => {
    ...
    expect(lobby.value).toEqual(lobbyParam); // Precisa atualizar a variável global com as informações do lobby fornecida pelo servidor
  });
  ...
  test('Deve gerar erro caso o jogador já estiver em um lobby', () => {
    ...
    lobby.value = lobbyParam; // Variável global de lobby definida marca que jogador já está em um lobby, e não poderia entrar em outro.
